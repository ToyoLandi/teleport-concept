...
Generate a private key for our user, replacing the <user> in brackets with their username.
```
...
curl https://raw.githubusercontent.com/ToyoLandi/teleport-concept/refs/heads/main/kubernetes/role.yaml -o roleBinding.yaml
```
...
base64 spezzy.key
```
> I like to copy these to a local notepad on my local machine to make this a little easier.

The Cluster Name can be fetched using... the default used in this guide is "kubernetes"
```
kubectl config view --minify -o jsonpath='{.clusters[].name}'
```

The last step is to copy the kubectl-config file to '~/.kube/config' for the user. If you wish to keep your sudo account with Admin access, you can create a new user with `useradd -m` and `mkdir ~./kube` before copying -OR- you can run the commands under [Running `kubectl` Commands with Admin Permissions] in the "installingK8s" to restore your Admin access.

## Last Words...
Kubernetes RBAC is capable of a lot of different things. We are just scratching the surface in this section. I STRONGLY recommend you read the [Kubernetes RBAC Docs](https://kubernetes.io/docs/reference/access-authn-authz/rbac/#api-overview) to learn more about the nuances and features that make RBAC so powerful. 

## What's Next?
We now have a new kubernetes user (spezzy) that is scoped to the webservers namespace, but that namespace does not yet exist, and the we have no web servers! Proceed to the next section to stand-up our namespace and give spezzy some work to do configuring NGINX with cert-manager.