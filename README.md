...
There are several sections to this guide intended to be followed in order, all documented under the "docs" directory to keep the README here more concise. If something isn't making sense, check the previous page in case you overlooked a step or note. Refer to the Table of Contents below for an ordered list of building this solution from scratch.
...
- An easily editable Ansible [inventory](https://github.com/ToyoLandi/teleport-concept/blob/main/ansible/playbooks/hosts.yaml) file to adapt the deployment to any LAN in a single-place.
- Deployed a Kubernetes Cluster with Two Worker Nodes and a Control-plane using `kubeadm` via Ansible Playbooks ([stage-k8s](https://github.com/ToyoLandi/teleport-concept/blob/main/ansible/playbooks/stage-k8s.yaml), [init-k8s](https://github.com/ToyoLandi/teleport-concept/blob/main/ansible/playbooks/init-k8s.yaml))
- Our Cluster will be configured...
    1. Using `containerd` as our Container Runtime Interface (CRI)
    2. Ensured containerd/runc will leverage [`systemd`](https://github.com/ToyoLandi/teleport-concept/blob/main/containerd/README.md) to properly track consumed CPU/Mem Resources across the Cluster & locally on the Kernel, preventing resource over-allocation.
...
- `cert-manager` deployed via `helm` in its own namespace so it cannot be tampered with by our normal user. 
- A normal user authenticating to `kubeapi` using Certificates signed by the Kubernetes CA generated by `kubeadm`, with [minimum role permissions](https://github.com/ToyoLandi/teleport-concept/blob/main/kubernetes/rbac/role.yaml), constrainted to the `webservers` namespace.
- A primitive `nginx` StaticSite deployed via [`helm`](https://github.com/ToyoLandi/teleport-concept/blob/main/helm/nginx-values.yaml) by our normal user, configured to...
    1. Pull the web-contents via `git` (no PVC needed)
    2. Use a certificate secret generated via `cert-manager` for TLS (HTTPS) by [`cert-manager.io.issuers`](https://github.com/ToyoLandi/teleport-concept/tree/main/kubernetes/cert-manager) - defined explictly in the `webservers` namespace should other certs be needed in the future for this namespace. 
    3. run with **two** replica's so we can host our website on each worker to maintain our service should one node go down.
- Verbose steps that can be referenced for future deployments.

#### Intentional Tradeoffs we have taken in this deployment...
